...
import { DeleteCategroies, FormatDate, GetMasterData } from '../utils/Const';
import { useDispatch, useSelector } from 'react-redux';
import { setMasterData } from '../Redux/Actions';

const QuestionsAddCat = () => {
    ...
    const handleDeleteCategory = (id) => {
        DeleteCategroies(id).then(async(d)=>{
            if(d!=null){
                let data = {
                    _id: Basic.uid,
    ...
    return (
        <>
            <AddCat2 open={open} setOpen={setOpen} type={"Questions"} />
...
export default QuestionsAddCat;
